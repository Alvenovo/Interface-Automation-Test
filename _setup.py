...
PROJ = r"C:\Users\王文东\Desktop\Interface-Automation-Test"

# ===== README.md =====
readme = """# Interface-Automation-Test
...
workflow_dir = os.path.join(PROJ, ".github", "workflows")
os.makedirs(workflow_dir, exist_ok=True)
test_yml = """name: 接口自动化测试
  ...
  test:
    ...
    steps:
      ...
      - name: 生成测试报告
        if: always()
        run: |
          python -m pytest tests/ -v --html=reports/report.html --self-contained-html --junitxml=reports/junit.xml
...
with open(os.path.join(workflow_dir, "test.yml"), "w", encoding="utf-8") as f:
    f.write(test_yml)
